...
int main(int argc, char** argv){
	...
	janela = glutCreateWindow("Ping-Pong");
...
void JanelaPlacar(void){
	...
	sprintf (label, "Computer Science");
	glRasterPos2f (-0.98F, 0.55F);
	DesenhaTexto (label);
	
	sprintf (label, "Press ESC - Close");
	glRasterPos2f (-0.98F, 0.0F);
	DesenhaTexto (label);
	sprintf (label, "Press ENTER - Start");
	glRasterPos2f (-0.98F, -0.55F);
	DesenhaTexto (label);
	...
	if(pts_jogador1==NUM_RODADAS){
		glColor3f(1.0, 1.0, 1.0);
		sprintf (label,  "Winner!");
		glRasterPos2f (0.45F, -0.50F);
		DesenhaTexto (label);
	}
	
	glColor3f(1.0, 1.0, 1.0);
	sprintf (label,  "Player 1");
